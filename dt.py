...
class Leaf(Node):
    ...
    def predict_class_probabilities(self, X: ArrayLike) -> np.ndarray:
        ...
        """
        # >>> YOUR CODE HERE >>>
        #就是计算先验概率,由于都是一样的所以我们返回的时候采用np.repeat
        n_sample = X.shape[0]
        probabilities = self.class_probabilities.reshape(1, -1)
        # <<< END OF YOUR CODE <<<
    ...
    def predict(self, X: ArrayLike) -> np.ndarray:
        ...
        labels = self.class_labels[max_prob_indices]
        # <<< END OF YOUR CODE <<<
        return labels
...
class DecisionTree:
    ...
    def _build_tree(self, X: ArrayLike, y: ArrayLike, max_depth: int, exclude: set = set()) -> Node:
        ...
        if max_depth <= 0 or self._is_pure(y) or len(exclude) == X.shape[1]:
            ...
            # Return a leaf node.
            # >>> YOUR CODE HERE >>>
            if max_depth <= 0 or len(exclude) == X.shape[1]:
                class_probabilities = self._compute_class_probabilities(y)
        ...
        # >>> YOUR CODE HERE >>>
        feature_of_split, splits = self.scorer.best_split(X, y, exclude)
        # <<< END OF YOUR CODE <<<
        ...
        if 'NA' not in children:
            # >>> YOUR CODE HERE >>>
            children['NA'] = Leaf(self._compute_class_probabilities(y))
            # <<< END OF YOUR CODE <<<
